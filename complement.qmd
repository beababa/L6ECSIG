...
table(df$dpt)
st_write(df, "data/cours3.gpkg", "commune", delete_layer = T)
```
...
st_write(inter, "data/cours3.gpkg", "car", delete_layer = T)
```
```{r}
commune <- st_read("data/cours3.gpkg", "commune", quiet = T)
commune <- commune [grep("97", commune$dpt),]
...
st_write(inter, "data/cours3.gpkg", "car97", delete_layer = T)
```


# fichiers cours 3

uniquement men_pauv et recodage
une couche par commune


on charge le 93
on affiche osm 
on charge commune
on sélectionne et on crée une nelle couche
on découpe car par commune
on décoche les couches
